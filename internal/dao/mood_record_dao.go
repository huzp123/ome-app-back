...
// Create 创建情绪记录
func (d *MoodRecordDAO) Create(mood *model.MoodRecord) error {
	return d.db.Create(mood).Error
}

// GetByID 根据ID获取情绪记录
func (d *MoodRecordDAO) GetByID(userID, moodID int64) (*model.MoodRecord, error) {
...
func (d *MoodRecordDAO) GetHistory(userID int64, startDate, endDate time.Time, limit int) ([]model.MoodRecord, error) {
...
func (d *MoodRecordDAO) GetTodayMoods(userID int64) ([]model.MoodRecord, error) {
	today := time.Now()
...
func (d *MoodRecordDAO) Delete(userID, moodID int64) error {
	result := d.db.Where("id = ? AND user_id = ?", moodID, userID).Delete(&model.MoodRecord{})
	if result.Error != nil {
		return result.Error
	}
...
func (d *MoodRecordDAO) GetMoodStatistics(userID int64, startDate, endDate time.Time) (map[string]interface{}, error) {
	...
	// 获取基本统计
	err := d.db.Model(&model.MoodRecord{}).
		Select(`
