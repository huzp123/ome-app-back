...
// Create 创建新用户
func (d *AppUserDAO) Create(user *model.AppUser) error {
	return d.db.Create(user).Error
}

// GetByID 根据ID获取用户
func (d *AppUserDAO) GetByID(id int64) (*model.AppUser, error) {
...
func (d *AppUserDAO) GetByPhone(phone string) (*model.AppUser, error) {
	var user model.AppUser
...
// GetByEmail 根据邮箱获取用户
func (d *AppUserDAO) GetByEmail(email string) (*model.AppUser, error) {
	var user model.AppUser
...
func (d *AppUserDAO) UpdateBasicInfo(id int64, heightCM float64, birthDate string, sex string) error {
	...
	return d.db.Model(&model.AppUser{}).
		Where("id = ?", id).
