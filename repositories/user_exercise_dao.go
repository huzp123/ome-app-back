...
// Create 创建运动记录
func (d *UserExerciseDAO) Create(exercise *models.UserExercise) error {
	return d.db.Create(exercise).Error
}

// GetByID 根据ID获取运动记录
func (d *UserExerciseDAO) GetByID(userID, exerciseID int64) (*models.UserExercise, error) {
...
func (d *UserExerciseDAO) GetHistory(userID int64, startDate, endDate time.Time, limit int) ([]models.UserExercise, error) {
	var exercises []models.UserExercise
...
func (d *UserExerciseDAO) GetTodayExercises(userID int64) ([]models.UserExercise, error) {
	today := time.Now()
...
// Update 更新运动记录
func (d *UserExerciseDAO) Update(exercise *models.UserExercise) error {
	return d.db.Save(exercise).Error
}

// Delete 删除运动记录
func (d *UserExerciseDAO) Delete(userID, exerciseID int64) error {
	result := d.db.Where("id = ? AND user_id = ?", exerciseID, userID).Delete(&models.UserExercise{})
	if result.Error != nil {
		return result.Error
	}
...
func (d *UserExerciseDAO) GetStatistics(userID int64, startDate, endDate time.Time) (map[string]interface{}, error) {
	...
	err := d.db.Model(&models.UserExercise{}).
		Select(`
