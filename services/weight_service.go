...
func NewWeightService(userWeightDAO *repositories.UserWeightDAO) *WeightService {
...
func filterDailyLastRecords(weights []models.UserWeight) []models.UserWeight {
	if len(weights) == 0 {
		return weights
	}

	// 使用map来存储每天的最后一条记录
	dailyLastRecord := make(map[string]models.UserWeight)
	...
	result := make([]models.UserWeight, 0, len(dailyLastRecord))
